...
public class PostsController(PostService _postService) : ControllerBase
{
    ...
    {
        if (!Enum.TryParse<PostVisibility>(addPostRequest.PostVisibility, true, out var visibility))
        {
            return BadRequest($"Invalid visibility value: {addPostRequest.PostVisibility}. Allowed values are public or private.");
        }
        await _postService.AddPost(addPostRequest.PostMessage, visibility);
        return Ok();
    }
}